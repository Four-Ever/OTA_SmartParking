...
//uint8 serializeMoveSignal(const move_signal* signal, uint8* buffer) {
//    buffer[0] = (signal->control_accel & 0x01) | ((signal->control_brake & 0x01) << 1);
//    buffer[1] = signal->control_steering_angle & 0x7F;  // 7비트 사용
//    buffer[2] = (signal->control_transmission & 0x03) | ((signal->test & 0x1F) << 2);
//    return 3;  // 바이트 크기 반환
//}
//uint8 getMoveSignalSize() {
//    return offsetof(CTRL_Move, signal) + sizeof(((CTRL_Move *)0)->signal);
//}
void SPIcommand()//uint8 *Txbuffer, uint8 *Rxbuffer)
{
//    uint8 Txbuffer[32];
//    uint8 Rxbuffer[32];
...
//    uint8 dlc =  getMoveSignalSize();
    ...
    msg.move_msg.signal.control_transmission = 1;
...
//    structToByteArray(&g_qspi.spiBuffers.spiMasterTxBuffer[1],&msg.move_msg,sizeof(msg.move_msg));
    g_qspi.spiBuffers.spiMasterTxBuffer[0] = dlc;
    g_qspi.spiBuffers.spiMasterTxBuffer[1] = msg.move_msg.msgId;
    memcpy(&g_qspi.spiBuffers.spiMasterTxBuffer[2], &msg.move_msg.signal, dlc);
