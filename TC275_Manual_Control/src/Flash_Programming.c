...
#define PFLASH_STARTING_ADDRESS     0xA0180000                  /* Address of the PFLASH where the data is written  */
...
#define PFLASH_NUM_SECTORS          2                           /* Number of PFLASH sectors to be erased            */
...
void writePFLASH(uint32 startingAddr,IfxMultican_Message* msgArr, uint8 cnt);
...
typedef struct
{
    ...
    void (*writeFlash)(uint32 startingAddr,IfxMultican_Message* msgArr, uint8 cnt);
} Function;
...
void erasePFLASH(uint32 sectorAddr)
{
    ...
    bb = g_commandFromPSPR.waitUnbusy(FLASH_MODULE, PROGRAM_FLASH_0);
}
...
void writePFLASH(uint32 startingAddr,IfxMultican_Message* msgArr, uint8 cnt)
{
    ...
    for(int offset = 0; offset < cnt; offset ++)     /* Loop over the page length                */
    {
        g_commandFromPSPR.load2X32bits(startingAddr, msgArr[offset].data[0], msgArr[offset].data[1]); /* Load 2 words of 32 bits each */
    }
...
void EraseProgramFlash(uint32 eraseAddr)
{
    ...
    /* Erase the Program Flash sector before writing */
    g_commandFromPSPR.eraseFlash(eraseAddr);
...
void WriteFirmware(uint32 startingAddr,IfxMultican_Message* msgArr, uint8 cnt) {
    g_commandFromPSPR.writeFlash(startingAddr,msgArr,cnt);
}
