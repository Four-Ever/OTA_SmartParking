...
#define LCF_INTVEC0_START   0x80129000
#define LCF_INTVEC1_START   0x8012A000
#define LCF_INTVEC2_START   0x80128000


#define LCF_TRAPVEC0_START  0x80064100
#define LCF_TRAPVEC1_START  0x8012B200
#define LCF_TRAPVEC2_START  0x8012B000
...
derivative tc27D
{
    ...
    memory BOOTA
    {
        ...
        size = 800k;
        type = rom;
        map     cached (dest=bus:sri, dest_offset=0x80064000,           size=800k);
        map not_cached (dest=bus:sri, dest_offset=0xa0064000, reserved, size=800k);
    }
...
#           include "inttab2.lsl"
    ...
    section_layout :vtc:abs18
    {
        group  (ordered, run_addr=mem:BOOTA)
        {
            select ".zrodata*";
        }
    }
    ...
    section_layout :vtc:linear
    {       
        group  bmh_0 (ordered, run_addr=0x80064000)
        {
            select "*.bmhd_0";
        }
        group  bmh_1 (ordered, run_addr=0x8011C000)
        {
            select "*.bmhd_1";
        }
        group  reset (ordered, run_addr=0x80064020)
        {
            select "*.boota_start";
        }
        group  interface_const (ordered, run_addr=0x80064040)
        {
            select "*.interface_const";
        }
        "__IF_CONST" := addressof(group:ainterface_const);
        group  a1 (ordered, run_addr=mem:BOOTA)
        {
            select ".srodata*";
            select ".ldata*";
        }
        ...
        group a8 (ordered, run_addr=mem:BOOTA)
        {
